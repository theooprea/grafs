...
void add_in_list(arbore_node **head, arbore_node *nod_nou)
{
    ...
    {
        /*arbore_node *iterator = *head;
        while(iterator->next != NULL)
        {
            iterator = iterator->next;
        }
        iterator->next = nod_nou;
        nod_nou->prev = iterator;*/
        nod_nou->next = (*head);
        (*head) = nod_nou;
    }
}
void make_arbore(arbore_node *root, int **labirint, int *miscari, int N, int M, int K)
{
...
int main()
{
    ...
    fisier_in = fopen("labirint1.in", "r");
