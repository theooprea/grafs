...
int Dijkstra(Graph *graf, int start, int destination, int *predecesori,
int *distance, int *selectat) {
    int i;
    list_node *aux;
...
void work_with_query(Graph *graf, int source,
int destination, FILE *fisier_out) {
    int aux = destination;
    ...
    int cost_minim = Dijkstra(graf, source, destination,
    predecesori, distance, selectat);
    ...
    if (index == 1) {
        ...
        for (i = 0; i < index; i++) {
            ...
            int cost_nou = Dijkstra(graf, source, destination,
            predecesori, distance, selectat);
